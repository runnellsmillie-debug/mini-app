// ==========================================
// APP.JS - Asosiy boshqaruv mantiqlari
// ==========================================
...
window.saveTx = (l, isDeepItem=false) => {
    ...
    window.amtStr = ""; if(de) de.value = ""; window.setTxt("num-display", "0"); window.save();
...
window.saveAndExit = () => { if(window.sessionData.length>0 && window.Telegram?.WebApp) window.Telegram.WebApp.sendData(JSON.stringify(window.sessionData)); else if(window.Telegram?.WebApp) window.Telegram.WebApp.close(); else window.toast("Saqlandi!"); };

// BARCHASINI UPDATE QILUVCHI ASOSIY RENDER
...
window.initApp = async function() {
    const raw = localStorage.getItem("xarajat_pro_v8");
    if (raw) { try { window.state = JSON.parse(raw); } catch(e) {} }
    if (!window.state.profiles || !window.state.profiles.length) window.state.profiles = [ { id: "general", name: "Umumiy", icon: "🏠", age: null, role: "parent_m" }, { id: "home_profile", name: "Uy/Ro'zg'or", icon: "🏡", age: null, role: "home" } ];
