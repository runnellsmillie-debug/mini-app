...
window.addScheduled = () => {
    ...
    if(!n||!d||d<1||d>31||!a) return window.toast(window.t("error_short"), true);
    ...
    window.save(); window.toast(window.t("added"));
};

window.delSched = id => { window.initCloseActionOld(() => { const s=window.state.sched.find(x=>x.id==id); if(s) { s.archived=true; s.closeDate = new Date().toISOString().slice(0,10); window.save(); window.toast(window.t("sched_archived")); } }); };
window.permDelSched = id => { window.state.sched=window.state.sched.filter(x=>x.id!=id); window.save(); window.toast(window.t("deleted_excl")); };
...
window.paySched = id => {
    ...
    window.save(); window.toast(window.t("saved") + " ✅");
};
...
window.renderSchedSet = function() {
    ...
    let h = active.map(s => `<div class="list-item"><span><b>${s.label}</b> (${s.day}${window.t("sched_day_suffix")})</span><button onclick="delSched(${s.id})" class="delete-btn">✕</button></div>`).join("");
...
window.saveDebt = () => {
    ...
    if(!n||!a) return window.toast(window.t("error_fill"), true);
...
window.closeDebt = id => { window.initCloseActionOld(() => { const d=window.state.debts.find(x=>x.id==id); if(!d) return; const t=new Date().toISOString().slice(0,10); if(d.type==="take") { window.state.incs.unshift({id:Date.now(), amount:d.amount, desc:`Qarz qaytdi: ${d.name}`, cat:"Qarz", date:t, time:"00:00", user:window.tgUser, prof:"general"}); if(window.creditIncomeToReserve) window.creditIncomeToReserve(d.amount, `Qarz qaytdi: ${d.name}`); } else window.state.txs.unshift({id:Date.now(), amount:d.amount, desc:`Qarz to'landi: ${d.name}`, cat:"Qarz", date:t, time:"00:00", user:window.tgUser, prof:"general"}); d.archived = true; d.closeDate = t; window.save(); window.toast(window.t("debt_close_btn")); }); };
window.permDelDebt = id => { window.state.debts=window.state.debts.filter(x=>x.id!=id); window.save(); window.toast(window.t("deleted_excl")); };
